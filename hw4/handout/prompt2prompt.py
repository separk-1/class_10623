...
def get_replacement_mapper_(x: str, y: str, tokenizer):
    ...
    ####TODO####
        ####TODO####
    mapper[0, 0] = 1  # Special token ([CLS] 등) 매핑
    ...
    for i in range(len(words_x)):
        if words_x[i] == words_y[i]:
            ...
            mapper[inds_x, inds_y] = 1.0
            mapper_last_x += len(inds_x)
            mapper_last_y += len(inds_y)

    changed_indices = [i for i in range(len(words_x)) if words_x[i] != words_y[i]]
    for idx in changed_indices:
        inds_x = get_word_inds(x, idx, tokenizer)
        inds_y = get_word_inds(y, idx, tokenizer)

        if len(inds_x) == len(inds_y):
            mapper[inds_x, inds_y] = 1.0
        elif len(inds_x) == 1:
            mapper[inds_x[0], inds_y] = 1.0 / len(inds_y)
        elif len(inds_y) == 1:
            mapper[inds_x, inds_y[0]] = 1.0 / len(inds_x)
        else:
            weight = 1.0 / max(len(inds_x), len(inds_y))
            for i in inds_x:
                for j in inds_y:
                    mapper[i, j] = weight
    ...
    leftover = min(max_len - mapper_last_x, max_len - mapper_last_y)
    for i in range(leftover):
        mapper[mapper_last_x + i, mapper_last_y + i] = 1.0
        ...
        if ((is_cross_attn and self.cur_step < self.num_steps_cross) or 
            ...
            if is_cross_attn:
                ...
                attn_probs[1:] = torch.einsum(
                    'b h n m, b m k -> b h n k',
                    attn_base.unsqueeze(0).expand(attn_replace.shape[0], *attn_base.shape),
                    self.mapper
                )
...
class MyCrossAttention(nn.Module):
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        attention_output = self.attn.batch_to_head_dim(attention_output)
        hidden_states = self.attn.to_out[0](attention_output)
        hidden_states = self.attn.to_out[1](hidden_states)

        if self.attn.residual_connection:
            hidden_states = hidden_states + residual

        if self.attn.rescale_output_factor != 1.0:
            hidden_states *= self.attn.rescale_output_factor
