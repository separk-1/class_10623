...
def get_replacement_mapper_(x: str, y: str, tokenizer):
    ...
    for i, (word_x, word_y) in enumerate(zip(words_x, words_y)):
        inds_x = get_word_inds(x, i, tokenizer)
        inds_y = get_word_inds(y, i, tokenizer)

        if word_x == word_y:
            mapper[np.ix_(inds_x, inds_y)] = 1.0
        elif len(inds_x) == len(inds_y):
            mapper[np.ix_(inds_x, inds_y)] = 1.0
        elif len(inds_x) == 1:
    ...
    pad = max_len - max(total_x, total_y)
    for i in range(pad):
        mapper[total_x + i, total_y + i] = 1.0
