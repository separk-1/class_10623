...
def get_data(batch_size):
    transform_img = T.Compose([
        ...
        T.Normalize(mean=[0.485], std=[0.229]), # Normalize each color dimension
        ])
...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.linear_relu_stack = nn.Sequential(
            nn.Linear(img_size[0] * img_size[1] * 1, 512),
            nn.ReLU(),
            nn.Linear(512, 512),
            nn.ReLU(),
...
def main(n_epochs, batch_size, learning_rate):
    ...
    for t in range(n_epochs):
        ...
        val_loss, val_acc = evaluate(test_dataloader, "validation", model, loss_fn)
