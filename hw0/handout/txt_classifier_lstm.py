...
class LSTMTextClassificationModel(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, num_layers, num_class, pad_idx):
        super(LSTMTextClassificationModel, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=pad_idx)
        self.lstm = nn.LSTM(embed_dim, hidden_dim, num_layers=num_layers, batch_first=True)
        self.pooling = nn.AdaptiveMaxPool1d(1)  # 패딩 영향을 줄이기 위해 사용
    ...
    def forward(self, text):
        ...
        pooled_out = self.pooling(lstm_out.permute(0, 2, 1)).squeeze(2)  # [batch_size, hidden_dim]
...
def main():
    wandb.finish()
    wandb.init(project="text-classification", name="LSTM Text Classification", reinit=True)
