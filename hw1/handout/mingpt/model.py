...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def __init__(self, d: int, base: int = 10_000):
        ...
        # Compute frequency terms once and store them
        half_dim = d // 2
        exponent = torch.arange(half_dim, dtype=torch.float32) / half_dim
        self.freqs = 1.0 / (self.base ** (torch.arange(half_dim, dtype=torch.float32) * 2 / self.d))
    ...
    def _build_cache(self, Y: torch.Tensor):
        ...
        positions = torch.arange(1, seq_len + 1, dtype=torch.float32, device=Y.device).unsqueeze(1)  # (seq_len, 1)

        self.freqs = self.freqs.to(Y.device)
        angles = positions * self.freqs  # Shape: (seq_len, half_dim)


        self.cos_cache = torch.cos(angles).unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, seq_len, half_dim)
        self.sin_cache = torch.sin(angles).unsqueeze(0).unsqueeze(0)  # Shape: (1, 1, seq_len, half_dim)
    ...
    def forward(self, Y: torch.Tensor):
        ...
        batch_size, num_heads, seq_len, dim = Y.shape
        half_dim = dim // 2

        # Compute cache if necessary
        if self.cos_cache is None or self.sin_cache is None or self.cos_cache.shape[2] < seq_len:
            self._build_cache(Y)

        # Split input into two halves
        Y_1, Y_2 = Y[..., :half_dim], Y[..., half_dim:]

        # Apply RoPE transformation
        Y_rotated = torch.cat(
            [Y_1 * self.cos_cache[:, :, :seq_len, :] - Y_2 * self.sin_cache[:, :, :seq_len, :],
            Y_1 * self.sin_cache[:, :, :seq_len, :] + Y_2 * self.cos_cache[:, :, :seq_len, :]],
            dim=-1
        )
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        assert config.n_embd % config.n_query_head == 0, "Embedding dimension must be divisible by query heads."
        assert config.n_query_head % config.n_kv_head == 0, "Query heads must be divisible by key-value heads."

        self.n_embd = config.n_embd
        self.n_query_head = config.n_query_head
        self.n_kv_head = config.n_kv_head
        self.dk = self.n_embd // self.n_query_head  # Head dimension
        self.group_size = self.n_query_head // self.n_kv_head  # Number of query heads per KV head
        ...
        self.q_proj = nn.Linear(self.n_embd, self.n_embd)  # (Batch, Seq_len, Embedding)
        self.k_proj = nn.Linear(self.n_embd, self.n_kv_head * self.dk)
        self.v_proj = nn.Linear(self.n_embd, self.n_kv_head * self.dk)
        ...
        self.rope = config.rope
        if self.rope:
            self.query_rotary_pe = RotaryPositionalEmbeddings(d=self.dk)  # Do NOT rename.
            self.key_rotary_pe = RotaryPositionalEmbeddings(d=self.dk)  # Do NOT rename.
    ...
    def forward(self, x):
        ...
        k = self.k_proj(x)  # (b, t, n_kv_head * dk)
        v = self.v_proj(x)  # (b, t, n_kv_head * dk)
        ...
        # Apply Rotary Positional Embeddings (RoPE) if enabled
        if self.rope:
            q = self.query_rotary_pe(q)
            k = self.key_rotary_pe(k)

        # Ensure that Key and Value match Query's head count
        k = k.unsqueeze(2).repeat(1, 1, self.group_size, 1, 1)  # Expand to match query heads
        k = rearrange(k, 'b h g t d -> b (h g) t d')  # Reshape to match query shape

        v = v.unsqueeze(2).repeat(1, 1, self.group_size, 1, 1)  # Expand to match query heads
        v = rearrange(v, 'b h g t d -> b (h g) t d')  # Reshape to match query shape

        # Free CUDA memory before allocation
        torch.cuda.empty_cache()
        start_memory = torch.cuda.memory_allocated()
        ...
        y = rearrange(y, 'b h q d -> b q (h d)')  # Merge heads
